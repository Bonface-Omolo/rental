...
<div class="center">
		<h2 style="margin-left: 50%;">Management Information</h2>
		<?php 
			$selectQuery = mysqli_query($con, "select * from user order by id desc"); 
			...
			?>
				...
				<tr>
					<form action="" method="post">
						...
						<td>
							<a href='user-edit.php?userid=<?php echo $row['id']; ?>' type="submit" name="edit" value="Edit" style="text-decoration:none;padding:3px;border-radius:4px;border:none;background-color:steelblue;color:white">Edit</a>
...
<?php 
	...
	if (isset($_POST['delete'])) {
		...
		$checkQuery = mysqli_query($con, "select * from user where id='$check' ");
		$noRow = mysqli_num_rows($checkQuery);

		if ($noRow>0) {
			$delete = mysqli_query($con, "delete from user where id='$check'");
			if ($delete) {
				echo "<script>alert('The row Deleted successfully'); window.open('admin-home.php','_self') </script>";
			}else{
